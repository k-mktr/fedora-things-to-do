...
def get_option_description(category: str, option: str) -> str:
    ...
    if category == "system_config":
        return nyattd_data[category][option]["description"]
    elif category in ["essential_apps", "additional_apps"]:
        return nyattd_data[category]["apps"][option]["description"]
    elif category == "customization":
        return nyattd_data[category]["apps"][option]["description"]
    else:
        raise ValueError(f"Unknown category: {category}")
...
def generate_options():
    nyattd_data = load_nyattd()
    logging.debug(f"generate_options - nyattd_data: {nyattd_data}")
    options = {
        "system_config": [key for key in nyattd_data["system_config"].keys() if key != "description"],
        "essential_apps": [app["name"] for app in nyattd_data["essential_apps"]["apps"]],
        "additional_apps": {},
        "customization": list(nyattd_data["customization"]["apps"].keys())
    }
    
    for category, data in nyattd_data["additional_apps"].items():
...
def build_system_config(options: Dict[str, Any], output_mode: str) -> str:
    ...
    nyattd_data = load_nyattd()
    system_config = nyattd_data["system_config"]
...
def build_app_install(options: Dict[str, Any], output_mode: str) -> str:
    ...
    # Essential apps
    essential_apps = [app for app in nyattd_data["essential_apps"]["apps"] if options["essential_apps"].get(app["name"], False)]
    if essential_apps:
    ...
    for category, category_data in options["additional_apps"].items():
        ...
        if category_apps:
            install_commands.append(f"# Install {nyattd_data['additional_apps'][category]['name']} applications")
            for app_id in category_apps:
                app_data = nyattd_data['additional_apps'][category]['apps'][app_id]
